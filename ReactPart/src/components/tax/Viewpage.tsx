...
const Viewpage: React.FC = () => {
 ...
 function showCheckbox(showColumnToggles: boolean){
  ...
  const columnDefs = [
    ...
    { headerName: 'Tax Name',      alignItems: "center",
    justifyContent: "center", flex: 2,field: 'tax_name', editable: true, filter: true, cellStyle: { textAlign: 'left' }, hide: !showColumns['tax_name'] },
    
    
    { headerName: 'Tax',      alignItems: "center",
    justifyContent: "center", flex: 2,field: 'tax', editable: true, filter: true, cellStyle: { textAlign: 'left' }, hide: !showColumns['tax'] },
  ...
  useEffect(() => {
    fetchData('/taxlistall/'+outlet_id, setCategoryData, setError, setIsLoading);
  ...
  const response = await fetch(`http://localhost:3000/api/updateTax/${id}`, {
    method: 'PUT',
    headers: {
