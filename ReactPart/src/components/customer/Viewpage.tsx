...
function formatDataToCSV(data: any[]): string {
  ...
  const csvHeaders = ['ID','Name','Mobile','Email','Loyality Points']; // Custom headings
  const csvRows = data && data.list && data.list.map((item: any) => {
    const name_full =item.customer_name!=''?item.customer_name:''+' '+item.last_name!=''?item.last_name:''
    const rowData = [
      item.id,
      name_full,
...
const Viewpage: React.FC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstancefile.post('/import-customer', formData);
...
const outlet_id = localStorage.getItem('outlet_id');
useEffect(() => {
fetchData('/customerList/'+outlet_id, setCategoryData, setError, setIsLoading);
...
const fetchDataAndDownloadCSV = () => {
  ...
  axios.get(`http://localhost:3000/api/customerList/${outlet_id}`, {
    headers: {
    ...
    .then(response => {
     ...
     console.log("------------------->"+JSON.stringify(data));
      ...
      downloadCSV(csvData, 'customer_data.csv'); // Function to trigger download
...
const generateAndDownloadPDF = (data: Customer[]) => {
  const pdf = new jsPDF();
  const headers = [['ID', 'Name', 'Mobile', 'Email', 'Loyalty Points']];
  const rows = data.map(item => [
    item.id,
    `${item.customer_name!='0'?item.customer_name:'--'} ${item.last_name!='0'?item.last_name:''}`,
    item.mobile!='0'?item.mobile:'--',
    item.email!='0'?item.email:'--',
    item.reward_points
  ]);
  ...
  <div className="flex flex-wrap sm:flex-nowrap justify-between items-center ">
                ...
                <NavLink to="/addcustomer" className="inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mr-4 mb-2" onClick={togglePopup}>
  ...
  <div className="sm:flex">
    <a  onClick={() => toggleModal()}  className=" cursor-pointer inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mr-4 sm-w-full mb-2">
      ...
      <span className="text-sm">Import Customer</span>
    </a>
  </div>
  ...
  <div className="sm:flex">
    <a onClick={togglePopup} className="inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mb-2">
      ...
      <span className="text-sm">Customer Filter</span>
    </a>
  </div>
</div>
    ...
    {categoryData && categoryData.list && categoryData.list.map((category: any) => (
      <tr key={category.id} className="whitespace-nowrap">
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
         ...
         <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><div className="h-12.5 w-15 rounded-md"> </div><p className="text-sm text-black dark:text-white">{category.customer_name}</p></div>
        </td>
        ...
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
          {category.address!='0'?category.reward_points:'---'}
        </td>
        ...
        <div className="relative w-auto max-w-md mx-auto my-6">
            ...
            <div className="relative bg-white w-full rounded-lg shadow-lg outline-none focus:outline-none">
              ...
              <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t">
                <h3 className="text-xl font-semibold">Import Customer</h3>
