...
const Viewpage: React.FC = () => {
 ...
 function showCheckbox(showColumnToggles: boolean){
  ...
  const columnDefs = [
    ...
    { headerName: 'Brand Name',      alignItems: "center",
    justifyContent: "center", flex: 2,field: 'brand_name', editable: true, filter: true, cellStyle: { textAlign: 'left' }, hide: !showColumns['brand_name'] },
  ...
  useEffect(() => {
    fetchData('/brandList/'+outlet_id, setCategoryData, setError, setIsLoading);
  ...
  const response = await fetch(`http://localhost:3000/api/updateBrand/${id}`, {
    method: 'PUT',
    headers: {
