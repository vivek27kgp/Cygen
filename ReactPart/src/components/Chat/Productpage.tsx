...
const Productpage: React.FC = () => {
  ...
  const fetchDataAndDownloadCSV = () => {
    ...
    axios
      .get(`http://localhost:3000/api/getAllItems/${outlet_id}`, {
        headers: {
      ...
      .then((response) => {
        ...
        downloadCSV(csvData, 'Product_data.csv'); // Function to trigger download
      })
      .catch((error) => {
  ...
  const fetchDataAndDownloadPDF = () => {
    ...
    axios
      ...
      .then((response) => {
        const data = response.data.items; // Assuming the response contains the data you want to export
  ...
  const generateAndDownloadPDF = (data: Category[]) => {
    ...
    const rows = data.map((item) => [
      item.id,
      `${item.item_name != '0' ? item.item_name : '--'}`,
      item.custom_barcode != '0' ? item.custom_barcode : '--',
      item.sku != '0' ? item.sku : '--',
      item.category_name != '0' ? item.category_name : '--',
      item.brand_name != '0' ? item.brand_name : '--',
      item.unit_name != '0' ? item.unit_name : '--',
      item.purchase_price != '0' ? item.purchase_price : '--',
      item.sales_price != '0' ? item.sales_price : '--',
      item.tax_name != '0' ? item.tax_name : '--',
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/getAllupdateStatus/${categoryId}`,
        {
          ...
          body: JSON.stringify({ status: newStatus, type: '2' }),
        },
      );
      if (!response.ok) {
      ...
      const updatedCategoryData = categoryData.items.map((category: any) => {
        if (category.id === categoryId) {
          return { ...category, status: newStatus };
        }
        return category;
      });
      setCategoryData({ ...categoryData, items: updatedCategoryData });
      // Display success toast

      toast.success(
        `Item ${newStatus == '0' ? 'Inactivated' : 'Activated'} successfully`,
        {
          autoClose: 3000,
          draggable: true,
          position: 'top-right',
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                          <div className="h-12.5 w-15 rounded-md">
                            ...
                            <img
                              width={50}
                              height={50}
                              src={`https://cygen.com.au/cygenwebretail/${category.item_image}`}
                              alt="Banner Image"
                            />{' '}
                          </div>
          ...
          <div className="fixed inset-y-0 right-0  flex justify-center items-end  newzindex overflow-y-auto">
            <div className="relative w-auto max-w-md mx-auto my-6">
              ...
              <div className="relative bg-white w-full rounded-lg shadow-lg outline-none focus:outline-none">
                ...
                {/* Modal Body */}
                <div
                  className="relative p-6 flex-auto overflow-x-auto overflow-y-auto h-94 scrollbar-thin scrollbar-thumb-bg-primary scrollbar-track-gray-100"
