...
function formatDataToCSV(data: any[]): string {
  ...
  const csvHeaders = ['ID','Name','Mobile','Email','address','Postcode']; // Custom headings
  const csvRows = data && data.list && data.list.map((item: any) => {
    const name_full =item.supplier_name!=''?item.supplier_name:''+' '+item.last_name!=''?item.last_name:''
    const rowData = [
      item.id,
      name_full,
...
const Viewpage: React.FC = () => {
  ...
  const fetchDataAndDownloadCSV = () => {
    ...
    axios.get(`http://localhost:3000/api/supplierlist/${outlet_id}`, {
      headers: {
      ...
      .then(response => {
       ...
       console.log("------------------->"+JSON.stringify(data));
        ...
        downloadCSV(csvData, 'Supplier_data.csv'); // Function to trigger download
  ...
  const generateAndDownloadPDF = (data: Supplier[]) => {
    const pdf = new jsPDF();
    const headers = [['ID', 'Name', 'Mobile', 'Email', 'Address', 'Postcode']];
    const rows = data.map(item => [
      item.id,
      `${item.supplier_name!='0'?item.supplier_name:'--'}`,
      item.mobile!='0'?item.mobile:'--',
      item.email!='0'?item.email:'--',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstancefile.post('/import-supplier', formData);
...
useEffect(() => {
fetchData('/supplierlist/'+outlet_id, setCategoryData, setError, setIsLoading);
...
const optionsProduct = [
  ...
  <div className="flex flex-wrap sm:flex-nowrap justify-between items-center ">
              <a onClick={fetchDataAndDownloadCSV} className="mr-4 mb-2 cursor-pointer">
              <img src={excelimg} alt="Excel" />
              </a>
              <a onClick={fetchDataAndDownloadPDF} className="mr-4 mb-2 cursor-pointer">
              <img src={pdfimg} alt="PDF" />
              </a>
                {/* Button to add a new product */}
                <NavLink to="/addsupplier" className="inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mr-4 mb-2" onClick={togglePopup}>
  ...
  <div className="sm:flex">
    <a  onClick={() => toggleModal()}  className=" cursor-pointer inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mr-4 sm-w-full mb-2">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-plus-circle mr-1 iconsize">
        <circle cx="12" cy="12" r="10" />
        <line x1="12" y1="8" x2="12" y2="16" />
        <line x1="8" y1="12" x2="16" y2="12" />
      </svg>
      <span className="text-sm">Import Supplier</span>
    </a>
  </div>
  ...
  <div className="sm:flex">
    {/* <a onClick={togglePopup} className="inline-flex items-center justify-center rounded-md border border-primary py-2 px-5 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10 mb-2">
      ...
      <span className="text-sm">Tax Filter</span>
    </a> */}
  </div>
</div>
    ...
    {categoryData && categoryData.list && categoryData.list.map((category: any) => (
      <tr key={category.id} className="whitespace-nowrap">
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
         ...
         <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><div className="h-12.5 w-15 rounded-md"> </div><p className="text-sm text-black dark:text-white">{category.supplier_name}</p></div>
        </td>
     
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
          {category.mobile!='0'?category.mobile:'---'}
        </td>
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
          {category.email!='0'?category.email:'---'}
        </td>
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
          {category.address!='0'?category.address:'---'}
        </td>
        <td className="px-6 py-4 text-xs sm:text-sm md:text-sm lg:text-sm xl:text-sm text-gray-500 text-center">
          {category.postcode!='0'?category.postcode:'---'}
        </td>
        ...
        <div className="relative w-auto max-w-md mx-auto my-6">
            ...
            <div className="relative bg-white w-full rounded-lg shadow-lg outline-none focus:outline-none">
              ...
              <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t">
                <h3 className="text-xl font-semibold">Import Supplier</h3>
