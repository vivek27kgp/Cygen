...
interface AddProps {
  ...
  const StepFour:  React.FC<AddProps> = ({ onSuccess, onError }) => {
  const store_insert_id = localStorage.getItem('store_insert_id');
  const outlet_id = localStorage.getItem('outlet_id');
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [app_store_status, set_app_store_status] = useState<any>('0'); 
  const [app_shipping_charges, set_app_shipping_charges] = useState<any>(null); 
  const [app_store_pickup_text, set_app_store_pickup_text] = useState<any>(null); 
  const [app_minimum_order_value, set_app_minimum_order_value] = useState<any>(null); 
  const [app_service_distance, set_app_service_distance] = useState<any>(null); 
  ...
  const handleToggle = () => {
    ...
    set_app_store_status(status);
    setIsToggled(!isToggled);
  };
  const [app_order_type, set_app_order_type] = useState<any>(null); 
  const handleChange_app_order_type = (event: React.ChangeEvent<HTMLSelectElement>) => {
    alert(event.target.value);
    set_app_order_type(event.target.value); // Update the selected option when it changes
 ...
 //   setError(null);
    ...
    try {
      ...
      const postData = {
        "app_store_status": app_store_status || 0,
        "outlet_id": outlet_id,
        "app_shipping_charges": app_shipping_charges || 0,
        "app_store_pickup_text": app_store_pickup_text || 0,
        "app_minimum_order_value": app_minimum_order_value || 0,
        "app_service_distance": app_service_distance || 0,
        "app_order_type":app_order_type||0
      };
      console.log(JSON.stringify(postData));
      if(store_insert_id!=''){
        alert(store_insert_id);
  ...
  <h1 className='border-b-2 border-primary w-1/3 p-2    text-title-sm'>App Setting</h1>
        <div className="p-6.5">
...
<div className={`w-1/2 xl:1/4 mb-1`}>
  &nbsp;<p className='smalltxt'>It will helps to ON/OFF the store</p>
        ...
        id="orderType"
        name={app_order_type}
        value={app_order_type} // Set the value of the select input to the selected option state
        onChange={handleChange_app_order_type} // Call handleChange function when the select input value changes
        ...
        <option value="3">Both Pickup & Delivery</option>
          ...
          <div className="flex flex-col gap-4 xl:flex-row">
            ...
            <div className={`w-full  mb-1`}>
              ...
              <input
                type="text"
                name="app_store_pickup_text"
                value={app_store_pickup_text}
                onChange={(e) => set_app_store_pickup_text(e.target.value)}
            ...
            <div className={`w-full  mb-1`}>
              ...
              <input
                ...
                value={app_shipping_charges}
                onChange={(e) => set_app_shipping_charges(e.target.value)}
          ...
          <div className="flex flex-col gap-4 xl:flex-row">
            <div className={`w-full  mb-1`}>
              ...
              <input
                type="text"
                name="app_minimum_order_value"
                value={app_minimum_order_value}
                onChange={(e) => set_app_minimum_order_value(e.target.value)}
            ...
            <div className={`w-full  mb-1`}>
              ...
              <input
                ...
                value={app_service_distance}
                onChange={(e) => set_app_service_distance(e.target.value)}
