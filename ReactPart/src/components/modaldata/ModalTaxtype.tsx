...
const ModalTax: React.FC<ModalProps> = ({ selectedData, closeModal }) => {
  ...
  const handleChange_category = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedCategory = categoryData.find((category: any) => category.id ==selectedCategoryId);

   // console.log("Selected Category:", selectedCategory);
    setCategory_name(selectedCategory?.id || ''); // Update the selected option when it changes
     closeModal(selectedCategory, selectedCategoryId,'6'); // Pass selected category data and ID to closeModal
     setIsModalOpen(false); // Close the modal
    };

  const categoryDataAll =[
    {'id':'','tax_type':"Select"},
    {'id':'Inclusive','tax_type':"Inclusive"},
    {'id':'Exclusive','tax_type':"Exclusive"}
  ]
  useEffect(() => {
    setCategoryData(categoryDataAll);
  }, []);
  ...
  <>
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 flex justify-center items-end z-999999">
        ...
        <div className="bg-white p-8 rounded-lg relative" style={{ width: '500px', height: '300px' }}>
          ...
          <div className="flex justify-right items-right mb-4">
            <button
              ...
              onClick={() => closeModal(selectedData, category_name,'6')} // Pass selected category data and ID when closing
            >
          ...
          <div className="w-full mb-1">
            <label className="mb-1.5 block text-black dark:text-white fotnlabel">Tax Type</label>
        ...
        {/* Modal overlay */}
        <div className="fixed inset-0 bg-black opacity-0 transition-opacity" onClick={() => closeModal(selectedData, category_name,'6')}></div>
      </div>
