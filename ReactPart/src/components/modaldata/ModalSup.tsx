...
const ModalSup: React.FC<ModalProps> = ({ selectedData, closeModal }) => {
  ...
  const handleChange_category = (event: React.ChangeEvent<HTMLSelectElement>) => {
   ...
   // console.log("Selected Category:", selectedCategory);
    setCategory_name(selectedCategory?.id || ''); // Update the selected option when it changes
     closeModal(selectedCategory, selectedCategoryId,'2'); // Pass selected category data and ID to closeModal
     setIsModalOpen(false); // Close the modal
    };

  useEffect(() => {
    fetchData('/getAllSuppliersoptions/' + outlet_id, setCategoryData, setError, setIsLoading);
  }, []);
  ...
  <>
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 flex justify-center items-end z-999999">
        ...
        <div className="bg-white p-8 rounded-lg relative" style={{ width: '500px', height: '300px' }}>
          ...
          <div className="flex justify-right items-right mb-4">
            <button
              ...
              onClick={() => closeModal(selectedData, category_name,'2')} // Pass selected category data and ID when closing
            >
        ...
        {/* Modal overlay */}
        <div className="fixed inset-0 bg-black opacity-0 transition-opacity" onClick={() => closeModal(selectedData, category_name,'2')}></div>
      </div>
