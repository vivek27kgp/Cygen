...
const ModalTax: React.FC<ModalProps> = ({ selectedData, closeModal }) => {
  ...
  useEffect(() => {
    fetchData('/getAllTaxOptions/' + outlet_id, setCategoryData, setError, setIsLoading);
  }, []);
  ...
  <>
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 flex justify-center items-end z-999999">
        ...
        <div className="bg-white p-8 rounded-lg relative" style={{ width: '500px', height: '300px' }}>
          ...
          <div className="flex justify-right items-right mb-4">
            <button
              ...
              onClick={() => closeModal(selectedData, category_name,'5')} // Pass selected category data and ID when closing
            >
          ...
          <div className="w-full mb-1">
            <label className="mb-1.5 block text-black dark:text-white fotnlabel">Tax</label>
        ...
        {/* Modal overlay */}
        <div className="fixed inset-0 bg-black opacity-0 transition-opacity" onClick={() => closeModal(selectedData, category_name,'5')}></div>
      </div>
